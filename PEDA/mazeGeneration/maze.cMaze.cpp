...
ERR_ENUM cMaze::init() {
    ...
    algorithms[0] = new cAlgorithmDFS;//<-------------|*
    return SUCCESS;
}
...
ERR_ENUM cMaze::create(size_t width, size_t height) {// clase para asignar memoria a matriz de celdas
    ...
    if (clear()==SUCCESS) {
        ...
        //}
        algorithms[0]->hookMaze(&maze,width,height);//<-------------|*
        return SUCCESS;
    } else {
        return FAILURE;
    }
...
ERR_ENUM cMaze::generate(void) {//<-------------|*
    x=99;
    y=99;
    //if (algorithms[algorithm]->generate()==SUCCESS) {// genera el algoritmo indicado en algorithm; 2 para DFS
    if (algorithms[0]->generate()==SUCCESS) {//<-------------|*
        return SUCCESS;
    } else {
        return FAILURE;
    }
